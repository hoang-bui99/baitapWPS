...
function mytheme_register_nav_menu()
{
    register_nav_menus(array(
        ...
        'footer_menu_1' => __('footer_menu_1', 'themeuniversity'),
        'footer_menu_2' => __('footer_menu_2', 'themeuniversity'),
    ));
}
...
function create_custom_post_types()
{
    ...
    $event_category_labels = array(
        'name' => _x('Danh mục Event', 'taxonomy general name', 'university'),
        'singular_name' => _x('Danh mục Event', 'taxonomy singular name', 'university'),
        'search_items' => __('Tìm kiếm Danh mục Event', 'university'),
        'all_items' => __('Tất cả Danh mục Event', 'university'),
        'parent_item' => __('Danh mục Event cha', 'university'),
        'parent_item_colon' => __('Danh mục Event cha:', 'university'),
        'edit_item' => __('Sửa Danh mục Event', 'university'),
        'update_item' => __('Cập nhật Danh mục Event', 'university'),
        'add_new_item' => __('Thêm Danh mục Event mới', 'university'),
        'new_item_name' => __('Tên Danh mục Event mới', 'university'),
        'menu_name' => __('Danh mục Event', 'university'),
    );
...
{
    add_meta_box('event_date_metabox', 'Ngày Event', 'event_date_metabox_callback', 'event', 'normal', 'high');
    add_meta_box('event_location_metabox', 'Địa điểm Event', 'event_location_metabox_callback', 'event', 'normal', 'high');
    add_meta_box('event_time_metabox', 'Thời gian Event', 'event_time_metabox_callback', 'event', 'normal', 'high');
}
...
function event_date_metabox_callback($post)
{
    ...
    echo '<label for="event_date">Ngày Event:</label>';
...
function event_location_metabox_callback($post)
{
    ...
    echo '<label for="event_location">Địa điểm Event:</label>';
...
function event_time_metabox_callback($post)
{
    ...
    echo '<label for="event_time">Thời gian Event:</label>';
...
function my_custom_login_logo_customizer($wp_customize)
{
    $wp_customize->add_section('my_custom_login_logo_section', array(
        'title' => __('Login Logo', 'university'),
        'priority' => 30,
    ...
    if (class_exists('WP_Customize_Image_Control')) {
        $wp_customize->add_control(new WP_Customize_Image_Control($wp_customize, 'my_custom_login_logo', array(
            'label' => __('Upload Login Logo', 'university'),
